...
- **2.8.2-pa.2** (PortfolioAtlas fork) - Added the PORTFOLIOATLAS VOICE RULES section (V1-V7): second-person openers, full sentences only, contractions, capped question-then-answer transitions (narrowing pattern 33), warmer plain diction, no editorial cleverness (subordinate to H1/H2), and the roundup top-ten-plus-one-table structure. Each rule carries a verbatim before/after pair from the owner's edit of the July 2026 $500K roundup.
- **2.8.2-pa.1** (PortfolioAtlas fork) - Forked blader/humanizer v2.8.2 for PortfolioAtlas: hard constraints H1-H7 (data integrity, no fabrication, content-type map, load-bearing hedges, domain vocabulary and house style, CLI output mode, voice calibration default).
